...
class TnConverter(object):
    ...
    def __init__(self, tn_tag=None, obs_tag=None, working_dir=None, 
                    output_dir=None, lang_code='en'):
        """
        :param tn_tag:
        :param obs_tag:
        :param working_dir:
    ...
    def run(self):
        ...
        self.filename_base = '{0}_obs-tn_v{3}'.format(self.lang_code, self.version)
    ...
    def setup_resource_files(self):
        if not os.path.isdir(os.path.join(self.working_dir, '{0}_obs-tn'.format(lang_code))):
            tn_url = self.get_resource_url('obs-tn', self.tn_tag)
            self.extract_files_from_url(tn_url)
        if not os.path.isdir(os.path.join(self.working_dir, '{0}_obs'.format(lang_code))):
            obs_url = self.get_resource_url('obs', self.obs_tag)
            self.extract_files_from_url(obs)
...
def main(tn_tag, obs_tag, lang_code, working_dir, output_dir):
    """
    :param tn_tag:
    :param obs_tag:
    :param lang_code:
    ...
    :return:
    """
    tn_converter = TnConverter(tn_tag, obs_tag, working_dir, output_dir, 
                                lang_code)
    tn_converter.run()
...
if __name__ == '__main__':
    ...
    if ! obs:
      obs = args.tn 
    main(args.tn, args.obs, args.lang_code, args.working_dir, args.output_dir)
