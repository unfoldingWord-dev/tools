...
for repo in "${repos[@]}"
do
     wkhtmltopdf --encoding utf-8 --outline-depth 3 -O portrait -L 15 -R 15 -T 15 -B 15  --header-html "file://$OUTPUT_DIR/${repo}-header.html" --header-spacing 5 --footer-html "$MY_DIR/footer.html" cover "file://$OUTPUT_DIR/${repo}-cover.html" toc --disable-dotted-lines --xsl-style-sheet "$TEMPLATE" "file://$OUTPUT_DIR/$repo.html" "$OUTPUT_DIR/$repo.pdf"
done
