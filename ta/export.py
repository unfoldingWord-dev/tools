...
'''
Exports tA for given language to specified format.
'''
...
def export(chapters_json, format, lang):
    global body_json
    '''
...
def getJSON(lang,entry,tmpent):
    ...
    anyJSONf = '/'.join([api_url_txt, anyJSONe])
...
def main(lang, outpath, format, checkinglevel):
    ...
    # Parse the body
    jsonf = 'ta-{0}.json'.format(lang)
    lang_url = '/'.join([api_url_txt, jsonf])
    tmpf = getJSON(lang, jsonf, '{0}-body-json.tmp')
    body_json = loadJSON(tmpf, 'd')
    checkForStandardKeysJSON()
    output = export(body_json['chapters'], format, lang)
    # For ConTeXt files only, Read the "main_template.tex" file replacing
    # all <<<[anyvar]>>> with its definition from the body JSON file
    if format == 'tex':
        outlist = []
