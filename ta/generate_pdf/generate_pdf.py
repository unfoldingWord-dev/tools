...
class TaConverter(object):
    ...
    def run(self):
        ...
        if True or self.regenerate or not os.path.exists(os.path.join(self.output_dir, '{0}.pdf'.format(self.file_id))):
...
def main(ta_tag, lang_code, working_dir, output_dir, owner, regenerate, logger):
    _print('Starting TA Converter for {0}...'.format(lang_code))
...
if __name__ == '__main__':
    ...
    for lang_code in lang_codes:
        main(args.ta, lang_code, args.working_dir, args.output_dir, args.owner, args.regenerate, logger)
