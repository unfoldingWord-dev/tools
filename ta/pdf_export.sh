...
# to the tA tools so these snippets can be found.
pushd "$BUILDDIR"
ln -sf "$BASEDIR/ta"
...
for lang in "${langs[@]}"; do
    ...
    ./ta/export.py -l $lang -f tex ${checking:+-c $checking} -o "$BASENAME.tex"

    # If requested, stop to manually edit the finished ConTeXt file before typesetting
    $edit && $EDITOR "$BASENAME.tex"

    # Run ConTeXt (context) to generate tA PDF from .tex file output by python
