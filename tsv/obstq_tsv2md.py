# -*- coding: utf-8 -*-
# This script converts OBS translation questions from TSV to the supported markdown format.
# It only processes a single folder or a single file, no recursion.
# The TSV files should be clean and completely correct format. (Use verify_obstqTSV.py)
#
# Column descriptions:
...
# Globals
source_dir = r'C:\DCS\Spanish-es-419\OBSTQ'  # Where are the TSV files located
...
# Returns the path of the markdown file to contain the questions for the specified paragraph
...
# Writes the specified questions to content/<story>/<paragraph>.md
def writeMdfile(story, paragraph, entries):
    ...
    for row in data:
        ...
        if story > 0:
            if story != prevstory or paragraph != prevparagraph:
                writeMdfile(prevstory, prevparagraph, entries)
                prevstory = story
                prevparagraph = paragraph
                entries = []
            entries.append( (row[5],row[6]) )
    writeMdfile(story, paragraph, entries)
