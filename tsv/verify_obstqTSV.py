# -*- coding: utf-8 -*-
# To verify proper format of each row in a TSV OBS-tQ file.
# Column descriptions:
#   1. "Reference" -- story:paragraph  e.g. 1:2 is the second paragraph in story 1.
#   2. "ID" -- unique 4-character ID
#   3. "Tags" (ignored) -- always blank
#   4. "Quote" (ignored) -- always blank
#   5. "Occurrence" (ignored) -- always blank
#   6. "Question"
#   7. "Response"
...
#   Question and Response checks. Some of these conditions are correctable with tsv_cleanup.py.
#      Blank values.
...
# Globals
source_dir = r'C:\DCS\Spanish-es-419\OBSTQ'
...
suppress12 = False    # Suppress warnings about markdown syntax
suppress14 = True    # Suppress warnings about mismatched TA page references (Only report total number)
suppress15 = False    # Suppress warnings each and every blank entry (Only report number of blank entries.)
...
class State:        # State information about a single Question and Response
...
def reportSuppressions():
    ...
    if suppress12:
        reportSuppression("markdown syntax in questions and responses")
    # if suppress14:
...
def checkTALinks(line):
    ...
    while page:
        ...
        if not os.path.isdir(path):
            reportError(f"invalid tA page: {manpage}")
...
# Verify tA links, note links, OBS links and internal links.
def checkLinks(line):
...
# Internal links point to another questions in the same OBS-tQ file.
...
# Column 6 (Question) or 7 (Response) verification
def verifyEntry(entry, type, id):
    state = State()

    if len(entry) == 0:
        state.incrementBlanks()
        if not suppress15:
            reportError(f"blank {type}")
    state.countParens(entry)
    if entry.find("<!--") != -1 or entry.find("&nbsp;") != -1 or entry.find("<b>") != -1 or entry.find("<span") != -1:
        reportError(f"html code in {type}")
    if "#" in entry:
        reportError(r"hash symbol in {type}")
    checkLinks(entry)
    reportParens()
    if len(entry) > 0 and entry.isascii() and not suppress9:
        reportError(f"Untranslated {type}")
    if unexpected_re.search(entry):
        reportError("found ']' after left paren")
    if unexpected2_re.search(entry):
...
def checkHeaderRow(row):
    ...
    checkColHeader(row[5], "Question", 7)
...
def checkRow(row):
    ...
    if state.story < 1 or (state.story == state.prevstory and state.paragraph < state.prevparagraph):
    ...
    verifyEntry(row[5].strip(), "question", row[1])
    verifyEntry(row[6].strip(), "response", row[1])
...
def verifyFile(path):
    ...
    for row in data:
        ...
        if nColumns > 1:
            ...
            reportError(f"Wrong number of columns ({nColumns})")
    if state.nBlanks > 0:
        reportError("has " + str(state.nBlanks) + " blank questions or responses", False)
